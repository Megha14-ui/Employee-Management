...
import { Box, TextField, IconButton, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Checkbox, Dialog, DialogTitle, DialogContent, DialogActions, Button, Pagination, CircularProgress, MenuItem, Select, Typography, Alert, InputAdornment, } from "@mui/material";
import { Search, FilterList, Add, Delete, Visibility, Edit, Clear, } from "@mui/icons-material";
const EmployeeList = () => {
    const navigate = useNavigate();
    const { employees, loading, error, deleteEmployee, deleteMultipleEmployees, clearError, } = useEmployeeContext();
    ...
    return (_jsxs(Box, { sx: { p: 3 }, children: [error && (_jsx(Alert, { severity: "error", onClose: clearError, sx: { mb: 2 }, children: error })), _jsxs(Paper, { sx: { p: 2, mb: 2, boxShadow: 3 }, children: [_jsxs(Box, { display: "flex", gap: 2, alignItems: "center", children: [_jsx(TextField, { fullWidth: true, placeholder: "Search employees...", value: searchTerm, onChange: (e) => setSearchTerm(e.target.value), InputProps: {
                    ...
                    setEmployeeToDelete(null);
                            ...
                            : "Are you sure you want to delete the selected employees?" }), _jsxs(DialogActions, { children: [_jsx(Button, { onClick: () => {
                                    setDeleteDialogOpen(false);
                                    setEmployeeToDelete(null);
